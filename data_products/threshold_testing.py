...
threshold_values1 = [0.90, 0.925, 0.95, 0.975, 1.00]
threshold_values2 = [1.30, 1.325, 1.35, 1.375, 1.40]
...
chd_image_list = [None] * len(threshold_values1)
...
for index, date in enumerate(query_times):
    ...
    if los_list[0] is not None:
        for t_index, thresh1 in enumerate(threshold_values1):
            plt.figure(str(date) + str(thresh1))
            chd_image_list[t_index] = chd_funcs.chd(iit_list, los_list, use_indices, inst_list, thresh1,
                                                    threshold_values2[t_index],
                                                    ref_alpha, ref_x, nc, iters)
            #### STEP FOUR: CONVERT TO MAP ####
            map_list[t_index], chd_map_list[
                t_index], methods_list, image_info, map_info = chd_funcs.create_singles_maps(inst_list,
                                                                                             date_pd,
                                                                                             iit_list,
                                                                                             chd_image_list[t_index],
                                                                                             methods_list,
                                                                                             map_x,
                                                                                             map_y, R0)
            ...
            euv_maps = []
            chd_maps = []
            for euv_map in map_list[t_index]:
                if len(euv_map.data) != 0:
                    euv_maps.append(euv_map)
            for chd_map in chd_map_list[t_index]:
                if len(chd_map.data) != 0:
                    chd_maps.append(chd_map)
            if del_mu is not None:
                euv_combined[t_index][index], chd_combined[t_index][index] = combine_maps(euv_maps, chd_maps,
                                                                                          del_mu=del_mu,
                                                                                          mu_cutoff=mu_cutoff)
            ...
            euv_combined[t_index][index].append_method_info(methods_list)
            euv_combined[t_index][index].append_method_info(pd.DataFrame(data=combined_method))
            euv_combined[t_index][index].append_image_info(image_info)
            euv_combined[t_index][index].append_map_info(map_info)
            chd_combined[t_index][index].append_method_info(methods_list)
            chd_combined[t_index][index].append_method_info(pd.DataFrame(data=combined_method))
            chd_combined[t_index][index].append_image_info(image_info)
            chd_combined[t_index][index].append_map_info(map_info)

            # plot maps
            euv_map_plot = euv_combined[t_index][index]
            chd_map_plot = chd_combined[t_index][index]
...
#             plt.contour(x_extent, chd_map_plot.y, chd_map_plot.data, origin="lower", cmap=chd_cmap,
            #                                          extent=[x_range[0], x_range[1], chd_map_plot.y.min(), chd_map_plot.y.max()],
            #                         linewidths=0.5, levels=1)
            # plt.imshow(chd_map_plot.data, extent=[x_range[0], x_range[1], euv_map_plot.y.min(), euv_map_plot.y.max()],
            #                        origin="lower", cmap=euv_cmap, aspect=90.0, norm=norm)

            plt.imshow(euv_map_plot.data, extent=[x_range[0], x_range[1], euv_map_plot.y.min(), euv_map_plot.y.max()],
                                            origin="lower", cmap=euv_cmap, aspect=90.0, norm=norm)
            plt.title("Date: " + str(date) + 'Thresholds: ' + str(thresh1) + ',' + str(threshold_values2[t_index]))
            #
            # , bbox_inches="tight"
            ...
            plt.plot(segments[:, 0], segments[:, 1], color=(1, 0, 0, .5), linewidth=0.3)
            plt.savefig('threshold/threshold_testing_' + str(date) + '_' + str(thresh1) + '.png')
