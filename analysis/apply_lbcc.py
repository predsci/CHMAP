...
from modules.DB_funs import init_db_conn, query_euv_images, query_lbcc_fit
...
# idea: new_data = Beta(mu)*los_ex.data + y(mu)

    # select image
    selected_image = image_pd.iloc[1]
    ...
    theoretic_query = query_lbcc_fit(db_session, image = selected_image, meth_name = "LBCC Theoretic")
    beta = theoretic_query[0]
    y = theoretic_query[1]
    # apply correction
...
###### PLOT ORIGINAL IMAGE ######
    Plotting.PlotImage(original_los, nfig = 0, title = "Original LOS Image")


##### PLOT CORRECTED IMAGE ######
    Plotting.PlotLBCCImage(lbcc_data = corrected_los_data, los_image = original_los, nfig = 1, title = "Corrected LBCC Image")
