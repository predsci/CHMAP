...
import analysis.ml_analysis.ml_functions as ml_funcs
...
for date_ind, center in enumerate(moving_avg_centers):
    ...
    if los_list[0] is not None:
        ...
        map_list, methods_list, data_info, map_info = image2map.create_singles_maps(inst_list, date_pd,
                                                                                         iit_list,
                                                                                         chd_image_list,
                                                                                         methods_list, map_x,
                                                                                         map_y, R0)
        #### STEP FIVE: CREATE COMBINED MAPS AND SAVE TO DB ####
        euv_combined = midm.create_combined_maps(db_session, map_data_dir, map_list,
