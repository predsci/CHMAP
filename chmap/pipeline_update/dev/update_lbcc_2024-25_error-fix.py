...
##############################################
# For November 2024 to April 2025, regenerate
# and overwrite AIA histograms
##############################################

# After Q died and then rose from the ashes of Edge,
# the AIA deconvolution routine quietly broke and
# started outputting images with the sun centered
# on the lower left corner of the image.  Once new
# processed images are created, this routine will
# fix the histograms and coefficients.
...
# max_im_date = datetime.datetime(2024, 11, 1, 0, 0, 0)
...
# In this case, we only need all AIA images from 2024-11-01
hist_images = db_funs.query_euv_images(db_session, time_min=datetime.datetime(2024, 11, 1, 0, 0, 0),
...
lbcc_coef_start = datetime.datetime(2024, 11, 1, 0, 0, 0) - window_del/2
...
for index, row in hist_images.iterrows():
    ...
    # add this histogram and meta data to database (mostly overwrite)
    db_funs.add_hist(db_session, hist_lbcc, overwrite=True)
